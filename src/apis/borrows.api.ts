...
import type { Borrow, CreateBorrowingRequest, UpdateBorrowingRequest, ReturnBookRequest } from "../types/borrows.type";
...
export const createBorrowing = async (
  ...
  console.log("API createBorrowing called with:", borrowing);
  console.log("Borrowing object keys:", Object.keys(borrowing));
  console.log("Borrowing userId:", borrowing.userId);
  console.log("Borrowing bookId:", borrowing.bookId);
...
export const returnBorrowing = async (
  ...
  const response = await api.post(`/borrowings/${borrowingId}/return`, returnData || {});
  return response.data;
};
